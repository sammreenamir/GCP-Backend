...
def visitor_counter(request):
    ...
    if request.method == 'GET':
        ...
        # Fetch the updated count after increment
        doc = doc_ref.get()
        if doc.exists:
            updated_count = doc.to_dict().get('count', 0)
        else:
            updated_count = 0
        
        # Return the updated count in the response
        response = make_response(jsonify({"message": "Visitor count updated", "count": updated_count}))
        response.headers = headers
        return response
